...
const Hero: React.FC = () => {
  ...
  const [qrType, setQrType] = useState<'google' | 'apple' | 'default'>('default');
  ...
  useEffect(() => {
    ...
    // Detect device type for QR code
    const userAgent = window.navigator.userAgent.toLowerCase();
    if (/android|windows/.test(userAgent)) {
      setQrType('google');
    } else if (/iphone|ipad|ipod|macintosh|mac os/.test(userAgent)) {
      setQrType('apple');
    } else {
      setQrType('default');
    }
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden flex items-center justify-center bg-sapient-dark">
        ...
        >
          <div className="w-80 bg-[#0F172A]/90 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-6 relative overflow-hidden">
            ...
            <div className="bg-white p-4 rounded-2xl mb-5 flex justify-center aspect-square shadow-inner relative z-0">
              {qrType === 'google' ? (
                <img 
                  src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://play.google.com/store/apps/details?id=com.iw.mint.app&pcampaignid=web_share" 
                  alt="Google Play Store QR" 
                  className="w-full h-full object-contain mix-blend-multiply"
                />
              ) : qrType === 'apple' ? (
                <img 
                  src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://apps.apple.com/in/app/mint-by-investwell/id1479042500" 
                  alt="Apple App Store QR" 
                  className="w-full h-full object-contain mix-blend-multiply"
                />
              ) : (
                <img 
                  src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://sapientfinserv.investwell.app/" 
                  alt="Download App QR" 
                  className="w-full h-full object-contain mix-blend-multiply"
                />
              )}
            ...
            <div className="text-center relative z-10">
              <h3 className="text-sapient-gold font-bold text-sm tracking-widest mb-2 uppercase">
                {qrType === 'google' ? 'Google Play Store' : qrType === 'apple' ? 'Apple App Store' : 'Seamless Integration'}
              </h3>
              <p className="text-gray-300 text-xs font-light">
                {qrType === 'google' && 'Scan to download from Google Play Store'}
                {qrType === 'apple' && 'Scan to download from Apple App Store'}
                {qrType === 'default' && 'Scan to experience the D&S Investment App'}
              </p>
            </div>
          </div>
        </div>
