  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 transition-colors duration-300">
      ...
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between py-4 md:py-6 relative z-50">
        ...
        >
          <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-sapient-teal to-sapient-teal-dark rounded-full flex items-center justify-center text-white font-bold text-xs md:text-sm shadow-lg group-hover:shadow-sapient-teal/50 transition-shadow duration-300 border border-white/10 shrink-0">
            D&S
          </div>
        ...
        <div className="flex items-center gap-2 md:gap-6 flex-shrink-0">
          ...
          <a 
            href="https://sapientfinserv.investwell.app/" 
            target="_blank"
            rel="noreferrer"
      ...
      {/* Mobile Menu Drawer */}
      <div 
        className={`fixed inset-0 z-40 bg-sapient-dark/95 backdrop-blur-xl transition-transform duration-300 md:hidden pt-24 px-6 overflow-y-auto ${
      ...
      >
        <div className="flex flex-col space-y-6">
          {NAV_ITEMS.map((item) => (
            <div key={item.label} className="border-b border-white/10 pb-4">
              {item.isDropdown ? (
                <button 
                  onClick={() => toggleDropdown(item.label)}
                  className="flex items-center justify-between w-full text-lg font-medium text-white hover:text-sapient-gold transition-colors"
                >
                  {item.label}
                  <ChevronDown 
                    size={18} 
          ...
          <div className="pt-4 pb-12">
            <p className="text-gray-500 text-xs mb-4 uppercase tracking-widest">Get in Touch</p>
            <div className="text-white text-sm space-y-2 font-light">
              <p>+91 22 1234 5678</p>
              <p>contact@dandsinvestment.com</p>
            </div>
          </div>
        </div>
      </div>
